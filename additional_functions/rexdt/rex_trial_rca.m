function [ecodeout, etimeout, spkchan, spk, arate, h, v, start_time, badtrial] = rex_trial_rca(name, trial, includeaborted)
...
if ~isempty(aidx)
    ...
    while (cnt112 <= length( aoffset )) && (atm < endofthistrial )
	%for ofst = 1:length(aoffset)
        ...
        if ((atm >= startofthistrial) && (atm < endofthistrial)) || (ofst && acont )
            foundanalogdata = 1;
            ofst = ofst + 1;
            if ofst == 1
                analog_time = atm;
